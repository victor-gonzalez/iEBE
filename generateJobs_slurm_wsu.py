...
CMD="sbatch -J iEBE-batch -q primary --mem=2G --array=1-$NJOBS --chdir=$WORKDIRBASE --time=$TIME $WORKDIRBASE/job.sh %s $NEVTPERJOB %s"
...
(cd job-$SLURM_ARRAY_TASK_ID
    ...
    (cd ./finalResults
        /wsu/home/gm/gm62/gm6269/PROJECTS/UrQMDparser/runUrQMDParser "urqmdCombined.txt" $SLURM_ARRAY_TASK_ID
...
"""
    sbatch -J watcher -q primary --mem=2G -d afterany:$JobID --time=%s --chdir=%s %s/watcher.sh %s %s %s %d
...
if initial_condition_type == 'pre-generated':
    ...
    if read_in_mode == 3 : # IP MUSIC events
        ...
        nevents = random.sample(range(total_nevents),numberOfJobs*numberOfEventsPerJob)
